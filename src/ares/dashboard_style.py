...
STYLE = """:root{--void:#15171a;--panel:#292d34;--panel2:#32373f;--line:#3f454e;--line2:#5b636f;
--fg:#f5f3ef;--fg2:#c8c3ba;--fg3:#ada598;
--ok:#9fd4b0;--ap:#f0c17c;--no:#efa79e;--acc:#a8c9e4;
--mono:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace}
*{box-sizing:border-box}
body{margin:0;background:var(--void);color:var(--fg);
font:15px/1.6 -apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Helvetica,Arial,sans-serif;
background-image:radial-gradient(circle at 12% -12%,rgba(168,201,228,.07),transparent 48%),
radial-gradient(circle at 94% 2%,rgba(240,193,124,.05),transparent 42%)}
a{color:var(--acc);text-decoration:none}a:hover{text-decoration:underline}
.bar{border-bottom:1px solid var(--line);background:rgba(21,23,26,.94);padding:.85rem 0}
...
.dot i{width:7px;height:7px;border-radius:50%;background:var(--ok);box-shadow:0 0 0 3px rgba(159,212,176,.18)}
...
.panel{background:var(--panel);border:1px solid var(--line);border-radius:10px;margin-bottom:1.6rem;overflow:hidden;box-shadow:0 1px 3px rgba(0,0,0,.32)}
...
.dl:hover{border-color:var(--acc);text-decoration:none;background:#353a43}
.dl .ext{font:700 .68rem/1 var(--mono);letter-spacing:.06em;padding:.42rem .5rem;border-radius:5px;
background:rgba(168,201,228,.18);color:var(--acc);flex:none}
...
.chip.demo{color:var(--ap);border-color:rgba(240,193,124,.5);background:rgba(240,193,124,.16)}
.chip.eval{color:var(--acc);border-color:rgba(168,201,228,.5);background:rgba(168,201,228,.16)}
...
.mode.demo{border-color:rgba(240,193,124,.55);background:rgba(240,193,124,.12);color:#f3cf9b}
.mode.eval{border-color:rgba(168,201,228,.55);background:rgba(168,201,228,.11);color:#cfe2f2}
...
.tag.v{background:rgba(159,212,176,.2);color:var(--ok)}
.tag.s{background:rgba(240,193,124,.2);color:var(--ap)}
.meta{color:var(--fg3);font:.72rem var(--mono);margin-top:.25rem}
.panel.aporia{border-color:rgba(239,167,158,.55)}
.panel.aporia .ph{background:rgba(239,167,158,.13);border-bottom-color:rgba(239,167,158,.34)}
...
button{background:var(--acc);color:#12151a;border:0;border-radius:6px;padding:.7rem 1rem;
font:700 .74rem/1 var(--mono);letter-spacing:.12em;cursor:pointer}
button:hover{filter:brightness(1.09)}
button.ghost{background:transparent;color:var(--ap);border:1px solid rgba(240,193,124,.58)}
button.ghost:hover{background:rgba(240,193,124,.14);filter:none}
.hint{color:var(--fg3);font-size:.75rem;margin:0}
.err{border-color:rgba(239,167,158,.6);background:rgba(239,167,158,.13);color:#f5c4bc;
...
.st.complete{color:var(--ok);border-color:rgba(159,212,176,.55);background:rgba(159,212,176,.16)}
.st.running{color:var(--ap);border-color:rgba(240,193,124,.55);background:rgba(240,193,124,.16)}
.st.queued{color:var(--fg3);border-color:var(--line2)}
.st.failed{color:var(--no);border-color:rgba(239,167,158,.55);background:rgba(239,167,158,.16)}
...
.sev.ERROR{background:rgba(239,167,158,.2);color:var(--no)}
.sev.WARNING{background:rgba(240,193,124,.2);color:var(--ap)}
.sev.INFO{background:rgba(168,201,228,.18);color:var(--acc)}
.cwe{display:inline-block;font:700 .64rem/1 var(--mono);color:var(--acc);
background:rgba(168,201,228,.17);border-radius:4px;padding:.28rem .42rem;margin-right:.45rem}
@media(max-width:640px){.two{grid-template-columns:1fr}}

/* ---- live progress ---- */
.prog{padding:1.15rem 1.2rem;border:1px solid rgba(240,193,124,.5);border-radius:10px;
background:rgba(240,193,124,.09);margin-bottom:1.6rem}
.prog .top{display:flex;align-items:baseline;gap:.7rem;margin-bottom:.8rem;flex-wrap:wrap}
.prog .stage{font:700 .74rem/1 var(--mono);letter-spacing:.14em;color:var(--ap);text-transform:uppercase}
.prog .count{margin-left:auto;font:700 .84rem/1 var(--mono);font-variant-numeric:tabular-nums;color:var(--fg2)}
.prog .track{height:12px;border-radius:6px;background:rgba(0,0,0,.3);overflow:hidden;position:relative}
.prog .fill{height:100%;border-radius:6px;background:linear-gradient(90deg,var(--ap),#f7d9a6);
transition:width .4s ease}
/* No total to divide by (reading a log, waiting on a model): sweep instead of
   pretending to know a percentage. CSS animation only - the CSP blocks script. */
.prog .fill.unknown{width:38%;animation:sweep 1.5s ease-in-out infinite;
background:linear-gradient(90deg,rgba(240,193,124,.25),var(--ap),rgba(240,193,124,.25))}
@keyframes sweep{0%{transform:translateX(-100%)}100%{transform:translateX(300%)}}
@media (prefers-reduced-motion:reduce){.prog .fill.unknown{animation:none;width:100%;opacity:.5}}
.prog .steps{display:flex;gap:.4rem;margin-top:.9rem;flex-wrap:wrap}
.prog .step{font:700 .58rem/1 var(--mono);letter-spacing:.08em;padding:.34rem .5rem;border-radius:4px;
border:1px solid var(--line2);color:var(--fg3)}
.prog .step.on{border-color:var(--ap);color:var(--ap);background:rgba(240,193,124,.14)}
.prog .step.past{border-color:rgba(159,212,176,.5);color:var(--ok)}
.prog .note{color:var(--fg2);font-size:.78rem;margin:.85rem 0 0}
.runs .st.running{position:relative}
.jobpct{font:.68rem var(--mono);color:var(--ap);margin-left:.4rem}
...
color:var(--fg2);font-size:.79rem;background:rgba(240,193,124,.08)}
...
.fr .fb{position:relative;height:34px;background:rgba(255,255,255,.055);border-radius:5px;overflow:hidden}
.fr .fb i{position:absolute;inset:0 auto 0 0;border-radius:5px;display:block;
background:linear-gradient(90deg,var(--acc),rgba(168,201,228,.55))}
.fr.s-verified .fb i{background:linear-gradient(90deg,var(--ok),rgba(159,212,176,.5))}
.fr.s-attack .fb i{background:linear-gradient(90deg,var(--ap),rgba(240,193,124,.5))}
...
border-left:3px solid var(--no);background:rgba(239,167,158,.1);border-radius:0 6px 6px 0}
...
box-shadow:0 0 0 5px rgba(240,193,124,.22)}
...
.tlist li.hit{background:rgba(240,193,124,.09)}
